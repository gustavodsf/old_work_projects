...
class nlp:
	...
	def __init__(self):
		...
		producao = ProducaoDAO.ProducaoDAO(writeLogFile)

		pool_sema = threading.BoundedSemaphore(value=1)
		
		self.generateID = GenerateID.GenerateID()

		dicionario = Dict.Dict(producao,pool_sema,writeLogFile)

		numberOfThread = 4;

		#Pega a quantidade de linhas do banco de dados
		rowCount =  producao.getTableRowCount()

		areaDeConhecimento = self.montaHashAreaCohecimento(producao)
		...
		for i in range(0,cycles):
			...
			rows = producao.getProducaoOffsetLimit(offset,offset*i)
			threads = []
			k=0
			while k < len(rows):
				for l in range(0,numberOfThread):
					if k < len(rows):
						thread = DataProcess.DataProcess(inputFiles.stopWordBannedList,inputFiles.stopWordConnectList,inputFiles.tagTrigram,inputFiles.tagBigram,producao,rows[k],pool_sema,self.generateID,areaDeConhecimento,dicionario)
			...
			producao.mysql.commit();
			if i > 0 and i%10==0:
				dicionario.saveStemAndWordDict(i)
		#fechando os arquivos de log que contem os erros de insercao de cada tabela
		producao.fileSource.close()
		producao.fileWord.close()
		producao.fileStem.close()
		dicionario.saveStemAndWordDict(i)
		
	def montaHashAreaCohecimento(self,producao):
		rows = producao.getAreasDoConhecimento();
