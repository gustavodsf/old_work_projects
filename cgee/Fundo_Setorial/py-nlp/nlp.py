...
class nlp:
	...
	def __init__(self):
		...
		fundoSetorial = FundoSetorialDAO.FundoSetorialDAO(writeLogFile)

		pool_sema = threading.BoundedSemaphore(value=1)
		
		self.generateID = GenerateID.GenerateID()

		dicionario = Dict.Dict(fundoSetorial,pool_sema,writeLogFile)

		numberOfThread = 4;

		#Pega a quantidade de linhas do banco de dados
		rowCount =  fundoSetorial.getTableRowCount()

		areaDeConhecimento = self.montaHashAreaCohecimento(fundoSetorial)
		...
		for i in range(0,cycles):
			...
			rows = fundoSetorial.getFundoSetorialOffsetLimit(offset,offset*i)
			threads = []
			k=0
			while k < len(rows):
				for l in range(0,numberOfThread):
					if k < len(rows):
						thread = DataProcess.DataProcess(inputFiles.stopWordBannedList,inputFiles.stopWordConnectList,inputFiles.tagTrigram,inputFiles.tagBigram,fundoSetorial,rows[k],pool_sema,self.generateID,areaDeConhecimento,dicionario)
			...
			fundoSetorial.mysql.commit();
			if i > 0 and i%10==0:
				dicionario.saveStemAndWordDict(i)
		#fechando os arquivos de log que contem os erros de insercao de cada tabela
		fundoSetorial.fileSource.close()
		fundoSetorial.fileWord.close()
		fundoSetorial.fileStem.close()
		dicionario.saveStemAndWordDict(i)
		
	def montaHashAreaCohecimento(self,fundoSetorial):
		rows = fundoSetorial.getAreasDoConhecimento();
