...
class nlp:
	...
	def __init__(self):
		...
		convenio = ConvenioDAO.ConvenioDAO(writeLogFile)
		...
		dicionario = Dict.Dict(convenio,semaStem,semaWord,writeLogFile)

		numberOfThread = 4;

		#Pega a quantidade de linhas do banco de dados
		rowCount =  convenio.getTableRowCount()
		...
		for i in range(0,cycles):
			...
			rows = convenio.getconvenioOffsetLimit(offset,offset*i)
			threads = []
			k=0
			while k < len(rows):
				for l in range(0,numberOfThread):
					if k < len(rows):
						thread = DataProcess.DataProcess(inputFiles.stopWordBannedList,inputFiles.stopWordConnectList,inputFiles.tagTrigram,inputFiles.tagBigram,convenio,rows[k],semaSource,semaStem,semaWord,self.generateID,dicionario)
