...
@Repository
public class StemDAOMidiaSocial implements Serializable{
	...
	public List<CloudVO> retrieveTop200Stem(){
		...
		Query query =  sessionFactory.getCurrentSession().createQuery("SELECT s FROM StemMidiaSocial s ORDER BY s.qtdStem DESC");
	...
	public long findStemByRadical(String radical){
		...
		Query query =  sessionFactory.getCurrentSession().createQuery("SELECT s FROM StemMidiaSocial s WHERE  s.stem = :stem");
