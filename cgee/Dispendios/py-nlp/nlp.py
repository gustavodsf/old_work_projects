...
class nlp:
	...
	def __init__(self):
		...
		dispendios = DispendiosDAO.DispendiosDAO(writeLogFile)
		...
		dicionario = Dict.Dict(dispendios,semaStem,semaWord,writeLogFile)

		numberOfThread = 4;

		#Pega a quantidade de linhas do banco de dados
		rowCount =  dispendios.getTableRowCount()
		...
		for i in range(0,cycles):
			...
			rows = dispendios.getdispendiosOffsetLimit(offset,offset*i)
			threads = []
			k=0
			while k < len(rows):
				for l in range(0,numberOfThread):
					if k < len(rows):
						thread = DataProcess.DataProcess(inputFiles.stopWordBannedList,inputFiles.stopWordConnectList,inputFiles.tagTrigram,inputFiles.tagBigram,dispendios,rows[k],semaSource,semaStem,semaWord,self.generateID,dicionario)
