...
class Dict:
	...
	def __init__(self,dispendiosDAO,semaStem,semaWord,writeLogFile):

		self.wordDict = dict()
		self.stemDict = dict()
		self.usedWordId = dict()
		self.dispendiosDAO = dispendiosDAO
	...
	def addWordDict(self,word,idSource,generateID,typeWord):
		...
		self.dispendiosDAO.savePalavraHasOrigem(idWordHasTxt,idSource,idWord)
	...
	def saveStemAndWordDict(self,cycle):
		...
		self.dispendiosDAO.saveWordDict(self.wordDict)
		self.writeLogFile.writeOnFileHashWord(self.wordDict)
		print ("..:: Salvando os radicais ::..")
		self.dispendiosDAO.saveStemDict(self.stemDict)
