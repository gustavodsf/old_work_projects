...
class DataProcess(threading.Thread):
	...
	def __init__(self,stopWordBannedList,stopWordConnectList,tagTrigram,tagBigram,producaoDAO,row,pool_sema,generateID,areaDeConhecimento,dicionario):
		...
		self.areaDeConhecimento = areaDeConhecimento
	...
	def run(self):
		...
		if txt != None and isinstance(txt,(str)): 
			...
			idArea = self.areaDeConhecimento[self.row[5]+self.row[6]]
			...
			self.producaoDAO.saveSource(self.row,idSource,idArea)
